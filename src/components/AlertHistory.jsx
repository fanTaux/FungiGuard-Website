    ...
    return (
        <section className={isDropdown ? "p-4" : "bg-white rounded-3xl p-8 border border-gray-100 shadow-sm overflow-hidden"}>
            <div className={`flex items-center justify-between ${isDropdown ? 'mb-4' : 'mb-8'}`}>
                <h2 className="font-bold text-[#624633] text-xl">Riwayat Aktivitas</h2>
                {!isDropdown && <span className="bg-[#fdfbf7] text-[#8c7462] px-3 py-1 rounded-full text-[10px] font-bold border border-[#f5e6d3]">LIVE</span>}
            </div>
                        ...
                        <div key={alert.id} className="py-4 flex items-start gap-4 group transition-all">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-baseline mb-1">
                                    <p className="font-bold text-[#624633] text-sm truncate uppercase tracking-tight">
            ...
            {!isDropdown && alerts.length > 0 && (
                <button className="w-full mt-6 py-3 border border-gray-100 rounded-2xl text-[10px] font-black text-[#8c7462] uppercase tracking-widest hover:bg-gray-50 transition-colors">
