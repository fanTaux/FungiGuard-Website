...
const Scan = {
  create: (data) => {
    const stmt = db.prepare(`
      INSERT INTO scan_history (deviceId, location, riskLevel, temperature, humidity, message, notes, duration, timestamp)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    return stmt.run(data.deviceId, data.location, data.riskLevel, data.temperature, data.humidity, data.message, data.notes, data.duration, new Date().toISOString());
  },
